...
from google.protobuf.internal.decoder import _DecodeVarint32  # noqa: PLC2701
from google.protobuf.internal.encoder import _EncodeVarint  # noqa: PLC2701
...
def make_demo_buffer(
    ...
    for idx, current_pair_info in enumerate(pair_infos):
        ...
        next_reward = 0
        next_reward = next_terminal_step.reward[0] if len(next_terminal_step) == 1 else next_decision_step.reward[0]
        current_obs = None
        if len(current_terminal_step) == 1:
            current_obs = next(iter(current_terminal_step.values())).obs
        else:
            current_obs = next(iter(current_decision_step.values())).obs
        ...
        if len(current_pair_info.action_info.continuous_actions) == 0 and len(current_pair_info.action_info.discrete_actions) == 0:
...
def get_demo_files(path: str) -> list[str]:
    ...
    if Path(path).is_file():
        ...
        return [path]
    if Path(path).is_dir():
        paths = [path / name for name in Path(path).iterdir() if name.suffix == ".demo"]
        if not paths:
...
def load_demonstration(
    ...
    for _file_path in file_paths:
        with Path(_file_path).open("rb") as fp:
            ...
            while pos < len(data):
                ...
                if obs_decoded == 0:
                    ...
                    if meta_data_proto.api_version not in SUPPORTED_DEMONSTRATION_VERSIONS:
                        raise RuntimeError(
...
def write_demo(demo_path, meta_data_proto, brain_param_proto, agent_info_protos):
    with Path(demo_path).open("wb") as f:
        # write metadata
