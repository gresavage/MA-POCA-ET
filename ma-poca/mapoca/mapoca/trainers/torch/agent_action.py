...
class AgentAction(NamedTuple):
    ...
    def slice(self, start: int, end: int) -> "AgentAction":
        ...
        if self.continuous_tensor is not None:
            cont = self.continuous_tensor[start:end]
        if self.discrete_list is not None and len(self.discrete_list) > 0:
            disc_list.extend(_disc[start:end] for _disc in self.discrete_list)
        return AgentAction(cont, disc_list)

    def to_action_tuple(self, clip: bool = False) -> ActionTuple:
        """Returns an ActionTuple."""
        action_tuple = ActionTuple()
        if self.continuous_tensor is not None:
            continuous_tensor = self.continuous_tensor
            if clip:
                continuous_tensor = torch.clamp(continuous_tensor, -3, 3) / 3
            continuous = ModelUtils.to_numpy(continuous_tensor)
    ...
    def from_buffer(buff: AgentBuffer) -> "AgentAction":
        ...
        if BufferKey.DISCRETE_ACTION in buff:
            discrete_tensor = ModelUtils.list_to_tensor(
                buff[BufferKey.DISCRETE_ACTION],
                dtype=torch.long,
            )
    ...
    def _group_agent_action_from_buffer(
        buff: AgentBuffer,
        cont_action_key: BufferKey,
        disc_action_key: BufferKey,
    ) -> list["AgentAction"]:
        """
        ...
        if cont_action_key in buff:
            ...
            continuous_tensors = [ModelUtils.list_to_tensor(arr) for arr in padded_batch]
        if disc_action_key in buff:
            padded_batch = buff[disc_action_key].padded_to_batch(dtype=np.long)
            discrete_tensors = [ModelUtils.list_to_tensor(arr, dtype=torch.long) for arr in padded_batch]

        actions_list = []
        for _cont, disc in itertools.zip_longest(
            continuous_tensors,
            discrete_tensors,
            fillvalue=None,
        ):
            if disc is not None:
                disc = [disc[..., i] for i in range(disc.shape[-1])]  # noqa: PLW2901
            actions_list.append(AgentAction(_cont, disc))
        return actions_list
