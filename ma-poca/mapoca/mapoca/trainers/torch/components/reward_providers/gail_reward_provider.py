...
from mapoca.trainers.settings import GAILSettings
    ...
    def evaluate(self, mini_batch: AgentBuffer) -> np.ndarray:
        with torch.no_grad():
            estimates, _ = self._discriminator_network.compute_estimate(
                mini_batch,
                use_vail_noise=False,
            )
            return ModelUtils.to_numpy(
                -torch.log(
                    1.0 - estimates.squeeze(dim=1) * (1.0 - self._discriminator_network.EPSILON),
                ),
            )

    def update(self, mini_batch: AgentBuffer) -> dict[str, np.ndarray]:
        expert_batch = self._demo_buffer.sample_mini_batch(
            mini_batch.num_experiences,
            1,
        )
...
class DiscriminatorNetwork(torch.nn.Module):
    ...
    def __init__(self, specs: BehaviorSpec, settings: GAILSettings) -> None:
        ...
        unencoded_size = self._action_flattener.flattened_size + 1 if settings.use_actions else 0  # +1 is for dones
        ...
        if settings.use_vail:
            ...
            self._z_sigma = torch.nn.Parameter(
                torch.ones((self.z_size), dtype=torch.float),
                requires_grad=True,
            )
            ...
            self._beta = torch.nn.Parameter(
                torch.tensor(self.initial_beta, dtype=torch.float),
                requires_grad=False,
            )
    ...
    def compute_loss(
        ...
        discriminator_loss = -(torch.log(expert_estimate + self.EPSILON) + torch.log(1.0 - policy_estimate + self.EPSILON)).mean()
        ...
        if self._settings.use_vail:
            ...
            kl_loss = torch.mean(
                -torch.sum(
                    1 + (self._z_sigma**2).log() - 0.5 * expert_mu**2 - 0.5 * policy_mu**2 - (self._z_sigma**2),
                    dim=1,
                ),
            )
        ...
        if self.gradient_penalty_weight > 0.0:
            gradient_magnitude_loss = self.gradient_penalty_weight * self.compute_gradient_magnitude(policy_batch, expert_batch)
    ...
    def compute_gradient_magnitude(
        ...
        interp_inputs = []
        for policy_input, expert_input in zip(policy_inputs, expert_inputs, strict=False):
        ...
        if self._settings.use_actions:
            ...
            policy_dones = torch.as_tensor(
                policy_batch[BufferKey.DONE],
                dtype=torch.float,
            ).unsqueeze(1)
            expert_dones = torch.as_tensor(
                expert_batch[BufferKey.DONE],
                dtype=torch.float,
            ).unsqueeze(1)
            dones_epsilon = torch.rand(policy_dones.shape)
            action_inputs = torch.cat(
                [
                    action_epsilon * policy_action + (1 - action_epsilon) * expert_action,
