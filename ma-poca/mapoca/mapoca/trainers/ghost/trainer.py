...
from mapoca.trainers.behavior_id_utils import BehaviorIdentifiers, create_name_behavior_id
...
from mapoca.trainers.training_status import GlobalTrainingStatus, StatusType
...
class GhostTrainer(Trainer):
    ...
    def __init__(
        ...
        self._team_to_name_to_policy_queue: defaultdict[
            int,
            dict[str, AgentManagerQueue[Policy]],
        ] = defaultdict(dict)
        ...
        self.play_against_latest_model_ratio = self_play_parameters.play_against_latest_model_ratio
        if self.play_against_latest_model_ratio > 1.0 or self.play_against_latest_model_ratio < 0.0:
            logger.warning(
        ...
        if self.steps_to_train_team > self.get_max_steps:
            logger.warning(
                f"The max steps of the GhostTrainer for behavior name {self.brain_name} is less than team change. This team will not face \
        ...
        self.policy_elos: list[float] = [self.initial_elo] * (self.window + 1)  # for learning policy
    ...
    def _process_trajectory(self, trajectory: Trajectory) -> None:
        ...
        """
        if trajectory.done_reached and trajectory.all_group_dones_reached and not trajectory.interrupted:
            # Assumption is that final reward is >0/0/<0 for win/draw/loss
            final_reward = trajectory.steps[-1].reward + trajectory.steps[-1].group_reward
            result = 0.5
            if final_reward > 0:
                result = 1.0
            elif final_reward < 0:
    ...
    def advance(self) -> None:
        ...
        for trajectory_queue in self.trajectory_queues:
            parsed_behavior_id = self._name_to_parsed_behavior_id[trajectory_queue.behavior_id]
            if parsed_behavior_id.team_id == self._learning_team:
                # With a future multiagent trainer, this will be indexed by 'role'
                internal_trajectory_queue = self._internal_trajectory_queues[parsed_behavior_id.brain_name]
                try:
        ...
        self._next_summary_step = self.trainer._next_summary_step  # noqa: SLF001
        ...
        for brain_name in self._internal_policy_queues:
            ...
            try:
                ...
                continue
            if self._learning_team == next_learning_team and next_learning_team in self._team_to_name_to_policy_queue:
                name_to_policy_queue = self._team_to_name_to_policy_queue[next_learning_team]
        ...
        if self._learning_team != next_learning_team or self.ghost_step - self.last_swap > self.steps_between_swap:
    ...
    def create_policy(
        ...
        if self.wrapped_trainer_team is None or team_id == self.wrapped_trainer_team:
            ...
            self.current_policy_snapshot[parsed_behavior_id.brain_name] = internal_trainer_policy.get_weights()
    ...
    def _save_snapshot(self) -> None:
        ...
        for brain_name in self.current_policy_snapshot:
            ...
            try:
                self.policy_snapshots[self.snapshot_counter][brain_name] = current_snapshot_for_brain_name
            except IndexError:
    ...
    def publish_policy_queue(self, policy_queue: AgentManagerQueue[Policy]) -> None:
        ...
        self._team_to_name_to_policy_queue[parsed_behavior_id.team_id][parsed_behavior_id.brain_name] = policy_queue
        if parsed_behavior_id.team_id == self.wrapped_trainer_team:
            ...
            self._internal_policy_queues[parsed_behavior_id.brain_name] = internal_policy_queue
    ...
    def subscribe_trajectory_queue(
        ...
        if parsed_behavior_id.team_id == self.wrapped_trainer_team:
            # With a future multiagent trainer, this will be indexed by 'role'
            internal_trajectory_queue: AgentManagerQueue[Trajectory] = AgentManagerQueue(parsed_behavior_id.brain_name)

            self._internal_trajectory_queues[parsed_behavior_id.brain_name] = internal_trajectory_queue
