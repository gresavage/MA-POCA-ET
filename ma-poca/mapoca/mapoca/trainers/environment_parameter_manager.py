...
from mapoca.trainers.settings import EnvironmentParameterSettings, ParameterRandomizationSettings
...
class EnvironmentParameterManager:
    def __init__(
        ...
        for parameter_name in self._dict_settings:
        ...
        for key in self._dict_settings:
    ...
    def get_minimum_reward_buffer_size(self, behavior_name: str) -> int:
        ...
        for settings in self._dict_settings.values():
            for lesson in settings.curriculum:
                if lesson.completion_criteria is not None and lesson.completion_criteria.behavior == behavior_name:
                    result = max(
                        result,
                        lesson.completion_criteria.min_lesson_length,
                    )
        return result
    ...
    def log_current_lesson(self, parameter_name: Optional[str] = None) -> None:
        ...
        if parameter_name is not None:
            ...
            logger.info(
                f"Parameter '{parameter_name}' is in lesson '{lesson_name}' and has value '{lesson_value}'.",
            )
        else:
            for parameter_name, settings in self._dict_settings.items():  # noqa: PLR1704
                lesson_number = GlobalTrainingStatus.get_parameter_state(
                    parameter_name,
                    StatusType.LESSON_NUM,
                )
    ...
    def update_lessons(
        ...
        for param_name, settings in self._dict_settings.items():
            ...
            lesson = settings.curriculum[lesson_num]
            if lesson.completion_criteria is not None and len(settings.curriculum) > next_lesson_num:
                ...
                if behavior_to_consider in trainer_steps:
                    must_increment, new_smoothing = lesson.completion_criteria.need_increment(
                        float(trainer_steps[behavior_to_consider]) / float(trainer_max_steps[behavior_to_consider]),
